...
def test_list_collections_returns_fixed_collections() -> None:
    payload = list_collections(StubCollectionRepository(), USER_CONTEXT)
...
def test_save_update_to_collection_is_idempotent_per_collection() -> None:
    ...
    first = save_update_to_collection(repository, USER_CONTEXT, "see-later", command, now=SAVED_AT)
    ...
    liked = save_update_to_collection(
        repository,
        USER_CONTEXT,
        "liked",
        command,
...
def test_unknown_collection_id_is_rejected_before_repository_write() -> None:
    ...
    with pytest.raises(CollectionNotFoundError, match="Collection not found"):
        remove_update_from_collection(repository, USER_CONTEXT, "unknown", "update-key")
...
def test_remove_update_from_collection_delegates_fixed_collection_id() -> None:
    ...
    remove_update_from_collection(repository, USER_CONTEXT, "liked", "update-key")
