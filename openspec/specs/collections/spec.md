...
The system SHALL provide the fixed collections `See Later`, `Liked`, and `History` independently for each authenticated user without requiring the user to create them.
...
- **THEN** the system returns `See Later`, `Liked`, and `History` collections for that user with stable ids and empty saved update lists

#### Scenario: Data store is initialized more than once
- **WHEN** the backend ensures the collection data store repeatedly
- **THEN** the system preserves exactly one row for each fixed collection for the authenticated user
